...
test("user onboarding", async ({ page, isMobile, menu, dismissToast }) => {
  ...
  await expect(page).toHaveTitle(/EuInvisto\.club/);
  await expect(page.getByText("Welcome to EuInvisto.club")).toBeVisible();
  ...
  await expect(page.getByText("Install EuInvisto.club")).toBeVisible();
