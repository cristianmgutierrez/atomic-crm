...
drop view if exists "public"."companies_summary";
  ...
  create table "public"."pipelines" (
    ...
    "pipeline_statuses" text[] not null default '{}'::text[],
...
alter table "public"."deals" add column "pipeline_id" bigint;
...
alter table "public"."deals" add constraint "deals_pipeline_id_fkey" FOREIGN KEY (pipeline_id) REFERENCES public.pipelines(id) not valid;

alter table "public"."deals" validate constraint "deals_pipeline_id_fkey";

alter table "public"."pipelines" add constraint "pipelines_escritorio_id_fkey" FOREIGN KEY (escritorio_id) REFERENCES public.escritorios(id) not valid;

alter table "public"."pipelines" validate constraint "pipelines_escritorio_id_fkey";
...
grant update on table "public"."pipelines" to "service_role";
  ...
  create policy "Pipelines delete"
  on "public"."pipelines"
  as permissive
  for delete
  to authenticated
using ((public.is_admin() OR (escritorio_id = public.get_my_escritorio_id())));



  create policy "Pipelines insert"
  on "public"."pipelines"
  as permissive
  for insert
  to authenticated
with check ((public.is_admin() OR (escritorio_id = public.get_my_escritorio_id())));



  create policy "Pipelines select"
  on "public"."pipelines"
  as permissive
  for select
  to authenticated
using ((public.is_admin() OR (escritorio_id = public.get_my_escritorio_id())));
  ...
  create policy "Pipelines update"
  on "public"."pipelines"
  as permissive
  for update
  to authenticated
...
CREATE TRIGGER set_pipeline_escritorio_id_trigger BEFORE INSERT ON public.pipelines FOR EACH ROW EXECUTE FUNCTION public.set_escritorio_id_default();

-- Data migration: create a default pipeline per escritório using existing config stages
DO $$
DECLARE
  existing_stages jsonb;
  existing_pipeline_statuses text[];
  esc RECORD;
BEGIN
  -- Read current deal stages and pipeline statuses from config (may not exist yet)
  SELECT
    COALESCE(config->'dealStages', '[{"value":"opportunity","label":"Opportunity"},{"value":"proposal-sent","label":"Proposal Sent"},{"value":"in-negociation","label":"In Negotiation"},{"value":"won","label":"Won"},{"value":"lost","label":"Lost"},{"value":"delayed","label":"Delayed"}]'::jsonb),
    ARRAY(SELECT jsonb_array_elements_text(COALESCE(config->'dealPipelineStatuses', '["won"]'::jsonb)))
  INTO existing_stages, existing_pipeline_statuses
  FROM public.configuration
  WHERE id = 1;

  -- Use defaults if config table is empty
  IF existing_stages IS NULL THEN
    existing_stages := '[{"value":"opportunity","label":"Opportunity"},{"value":"proposal-sent","label":"Proposal Sent"},{"value":"in-negociation","label":"In Negotiation"},{"value":"won","label":"Won"},{"value":"lost","label":"Lost"},{"value":"delayed","label":"Delayed"}]'::jsonb;
    existing_pipeline_statuses := ARRAY['won'];
  END IF;

  -- Create one default pipeline per escritório
  FOR esc IN SELECT id FROM public.escritorios LOOP
    INSERT INTO public.pipelines (name, stages, pipeline_statuses, position, escritorio_id)
    VALUES ('Funil Principal', existing_stages, existing_pipeline_statuses, 0, esc.id);
  END LOOP;

  -- Assign all existing deals to their escritório's new default pipeline
  UPDATE public.deals d
  SET pipeline_id = p.id
  FROM public.pipelines p
  WHERE p.escritorio_id = d.escritorio_id
